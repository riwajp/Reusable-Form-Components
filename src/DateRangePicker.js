...
const DateRangePicker = ({
  ...
  const dates = useWatch({ control, name: name });
  ...
  useEffect(() => {
    setValue(name + ".start", start_date);

    setValue(name + ".end", end_date);
  }, []);

  const handleChange = (date_type, t_start_date, t_end_date) => {
    if (date_type === "start") {
      setValue(name + ".start", new Date(t_start_date.setHours(0, 0, 0, 0)));
    } else {
      setValue(name + ".end", new Date(t_end_date.setHours(0, 0, 0, 0)));
    }
  };
